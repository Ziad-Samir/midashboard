...
def load_clean_data():
    ...
    cards = dbc.Row([
    dbc.Col(dbc.Card(dbc.CardBody([html.H5("Total Patients"), html.H2(f"{total_patients}")]), color="primary", inverse=True, className="zoom-in "), md=4),
    dbc.Col(dbc.Card(dbc.CardBody([html.H5("Average Age"), html.H2(f"{avg_age}")]), color="#335c8b8f", inverse=True, className="zoom-in "), md=4),
    dbc.Col(dbc.Card(dbc.CardBody([html.H5("Average Length of Stay (days)"), html.H2(f"{avg_stay}")]), color="#335c8b", inverse=True, className="zoom-in"), md=4),
    ]) 
     # Pie chart: Gender
...
# Text appearance
    ...
    return  df, fig_gender, revenue_bar, cards